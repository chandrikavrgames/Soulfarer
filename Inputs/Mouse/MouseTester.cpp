...
// Called every frame
void AMouseTester::Tick(float DeltaTime)
{
	...
	if (UsingRig) {
		//UE_LOG(LogTemp, Display, TEXT("Using rig"));
		//SetZVelocityRig();
		SetXVelocityRig();
		SetYVelocityRig();
	}
...
void AMouseTester::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (!UsingRig) {
		...
		//PlayerInputComponent->BindAxis("MouseX", this, &AMouseTester::setXVelocity);
		PlayerInputComponent->BindAxis("MouseY", this, &AMouseTester::setYVelocity);
	}


}
void AMouseTester::setXVelocity(float x) {
...
void AMouseTester::setYVelocity(float y) {
...
void AMouseTester::SetZVelocityRig() {
	if (usingLeft) {
...
void AMouseTester::SetXVelocityRig() {
	SerialData = UpdateSerialData(UCustomDataStructures::Mouse);
	hand_velocity.X = -SerialData.Velocities.X;
	//if (abs(SerialData.Velocities.Z) > 0.0) {
	//	UE_LOG(LogTemp, Display, TEXT("Z: %f, %f"), hand_velocity.Z, abs(SerialData.Velocities.Z));
	//}
	updateHandLocationVector();
...
void AMouseTester::SetYVelocityRig() {
	SerialData = UpdateSerialData(UCustomDataStructures::Mouse);
	
	hand_velocity.Z = -SerialData.Velocities.Y;
	//UE_LOG(LogTemp, Display, TEXT("Z: %f, %f"), hand_velocity.Z, abs(-SerialData.Velocities.Y));
...
void AMouseTester::updateHandLocationVector() {
...
void AMouseTester::zeroVelocityList() {
	for (int i = 0; i < maxVelList; i++) {
		arr_velocities[i] = (FVector(0, 0, 0));
	}
}

void AMouseTester::switchHands() {
	usingLeft = !usingLeft;
	if (usingLeft) {
		startPullPosition = startPullPosition_left;
		endPullPosition = endPullPosition_left;
